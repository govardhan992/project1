     ...
     stages{
        ...
        stage("Deploy container in server"){
            ...
            sshagent(['deploy_app']) {
              ...
              sh "ssh -o StrictHostKeyChecking=no root@172.31.37.150 docker pull public.ecr.aws/e5k4j6y8/test-ecr:\${BUILD_NUMBER}"
              sh "ssh -o StrictHostKeyChecking=no root@172.31.37.150 docker rm -f container2 || true"
              sh "ssh -o StrictHostKeyChecking=no root@172.31.37.150 docker run -d -p 8090:80 --name container2 public.ecr.aws/e5k4j6y8/test-ecr:\${BUILD_NUMBER}"
